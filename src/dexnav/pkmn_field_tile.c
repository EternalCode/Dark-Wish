...
u8 get_distance_player(s16 x, s16 y) {
    u16 delta_x = ABS(x - (gSaveBlock1Ptr->pos.x + 7));
    u16 delta_y = ABS(y - (gSaveBlock1Ptr->pos.y + 7));
...
u8 pick_tile_screen(u8 target_behaviour, u8 area_x, u8 area_y, s16 *x_buff, s16 *y_buff, u8 find_chance, u8 small_scan) {
    ...
    s16 top_x = gSaveBlock1Ptr->pos.x - SCANSTART_X + (small_scan * 5);
    s16 top_y = gSaveBlock1Ptr->pos.y - SCANSTART_Y + (small_scan * 5);
    ...
    while (top_y < bot_y) {
        while (top_x < bot_x) {
            ...
            for (u8 i = 0; i < NPC_STATE_ID_MAX; i++) {
                if (gMapObjects[i].currentCoords.x == top_x && gMapObjects[i].currentCoords.y == top_y) {
                    go_next = true;
                    break;
                }
            }
            ...
            if ((target_behaviour == block_properties)){// ) {
                ...
                if (target_behaviour == 2) {
                    ...
                    weight = (((rand() % scale) <= 1) && (cur_mapdata_height_mismatch(gMapObjects[gPlayerAvatar.spriteId].elevation, top_x, top_y))) ;
        ...
        top_y += 1;
        top_x = gSaveBlock1Ptr->pos.x - SCANSTART_X + (small_scan * 5);
    }
    return 0;
}
