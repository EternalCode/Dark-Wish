...
void pick_battle_message(u16 moveId, u8 user_bank, enum BattleTypes battle_type, enum battle_string_ids id, u16 move_effect_id)
{
    ...
    if (battle_type_flag == BATTLE_MODE_WILD) {
        switch (id) {
            ...
            case STRING_IN_LOVE:
            case STRING_BROKE_FREE:
            case STRING_APPEARED_CAUGHT:
            case STRING_ALMOST_HAD_IT:
            case STRING_SO_CLOSE_TOO:
            case STRING_GOTCHA_CAUGHT:
            case STRING_TRYING_TO_LEARN_MOVE:
            case STRING_FORGET_WHICH_MOVE:
            case STRING_STOP_LEARNING_MOVE:
            case STRING_CONFIRM_FORGET:
            case STRING_FORGOT_SELECTED_MOVE:
            case STRING_DISABLED_USED:
                fdecoder_battle(battle_strings[id], user_bank, moveId, move_effect_id);
                break;
