...
{
    p->x = gMapObjects[gPlayerAvatar.spriteId].currentCoords.x;
    p->y = gMapObjects[gPlayerAvatar.spriteId].currentCoords.y;
    p->height = gMapObjects[gPlayerAvatar.spriteId].elevation;
}
...
bool ProcessPlayerFieldInput(struct FieldInput *input)
{
    ...
    u8 playerDirection = gMapObjects[gPlayerAvatar.spriteId].direction & 0xF;
